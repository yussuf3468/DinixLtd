...
export default function ClientList({ onSelectClient }: ClientListProps) {
  ...
  const handleAddClient = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Generate unique client code using timestamp and random number
      ...
      let clientCode = generateUniqueClientCode();
      let attempts = 0;
      ...
      while (attempts < maxAttempts) {
        ...
        if (checkError) {
          console.error("Error checking client code:", checkError);
          toast.error("Failed to generate client code. Please try again.");
          return;
        }

        // If no existing client with this code, we're good
        if (!existing) {
          break;
        }
      ...
      if (attempts >= maxAttempts) {
        toast.error("Failed to generate unique client code. Please try again.");
        return;
      }
    ...
    <div className="p-3 sm:p-4 md:p-8">
      ...
      <div className="mb-6 sm:mb-8 relative">
        <button
          ...
          className="flex items-center justify-center w-11 h-11 sm:w-12 sm:h-12 bg-emerald-600 text-white rounded-full active:opacity-80 absolute right-0 top-0"
          aria-label="Add client"
        >
          <Plus className="w-5 h-5 sm:w-6 sm:h-6" />
        </button>

        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 pr-12 sm:pr-16">
          <div className="flex-1">
            <div className="flex items-center gap-2 sm:gap-3 mb-2">
              <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-xl flex items-center justify-center shadow-sm flex-shrink-0">
            ...
            <p className="text-xs sm:text-sm text-gray-600 ml-10 sm:ml-12">
              Manage and track all your clients • Dinix
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:gap-4 xl:gap-6 mb-4 sm:mb-6">
          <div className="bg-white rounded-xl p-4 sm:p-6 border border-gray-200 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-blue-500 to-cyan-600 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm">
        ...
        <div className="bg-white rounded-xl p-3 sm:p-4 border border-gray-200 shadow-sm space-y-3 sm:space-y-4">
          <div className="flex flex-col sm:flex-col md:flex-row gap-3 sm:gap-4">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
              ...
              <select
                ...
                className="flex-1 md:flex-none px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 bg-white text-gray-900 font-semibold hover:border-gray-300 transition-all"
              >
      ...
      {filteredClients.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center shadow-sm border border-gray-200">
          <div className="w-24 h-24 mx-auto mb-6 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-full flex items-center justify-center shadow-md">
          ...
          {!searchTerm && statusFilter === "all" && (
            <button
              onClick={() => setShowAddModal(true)}
              className="inline-flex items-center gap-2 px-8 py-4 bg-emerald-600 text-white rounded-xl active:opacity-80 font-medium"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
          {filteredClients.map((client) => {
            ...
            return (
              <div
                key={client.id}
                className="group bg-white rounded-2xl shadow-sm border border-gray-200 hover:shadow-md hover:border-emerald-500/40 transition-all duration-300 overflow-hidden"
              >
                {/* Status Indicator */}
                <div
                  className={`h-1 ${
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3 flex-1">
                      <div
                        className={`w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-sm bg-gradient-to-br ${
                          client.status === "active"
                  ...
                  <div className="pt-3 border-t border-gray-200 flex gap-2">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-emerald-600 text-white rounded-lg text-xs font-medium active:opacity-80"
                    >
                    ...
                    <button
                      ...
                      className="px-3 py-2 bg-red-50 border border-red-200 text-red-600 rounded-lg text-xs font-medium hover:bg-red-100 transition-all duration-200 active:scale-95"
                      title="Delete Client"
                    >
            ...
            <form onSubmit={handleAddClient} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  name="client_name"
                  required
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 bg-white text-gray-900 placeholder-gray-500 hover:border-gray-300 transition-all font-medium"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    defaultValue="KES"
                    className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 bg-white text-gray-900 hover:border-gray-300 transition-all font-medium"
                  >
              ...
              <div className="flex gap-3 pt-4">
                <button
                  type="button"
                  onClick={() => setShowAddModal(false)}
                  className="flex-1 px-6 py-3 border border-gray-200 rounded-xl text-gray-700 font-bold hover:bg-gray-100 hover:border-gray-300 transition-all active:scale-95"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 px-6 py-3 bg-emerald-600 text-white rounded-xl font-bold active:opacity-80 disabled:opacity-50 disabled:cursor-not-allowed"
                >
            ...
            <div className="p-6">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 px-6 py-3 bg-red-600 text-white rounded-xl font-bold active:opacity-80"
                >
                  ✓ Delete
                </button>
              </div>
